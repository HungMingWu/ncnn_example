...
std::vector<orbwebai::face::TrackedInfo> Tracker::Track(const std::vector<orbwebai::face::Info>& curr_faces) {
    std::cout << "start track face." << std::endl;
    int num_faces = static_cast<int>(curr_faces.size());

    std::deque<orbwebai::face::TrackedInfo> scored_tracked_faces(pre_tracked_faces_.begin(), pre_tracked_faces_.end());
    std::vector<orbwebai::face::TrackedInfo> curr_tracked_faces;
    for (int i = 0; i < num_faces; ++i) {
        ...
        if (!scored_tracked_faces.empty() && scored_tracked_faces.front().iou_score_ > minScore_) {
			orbwebai::face::TrackedInfo matched_face = scored_tracked_faces.front();
			scored_tracked_faces.pop_front();
			orbwebai::face::TrackedInfo &tracked_face = matched_face;
